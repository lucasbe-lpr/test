...
/* DÉCOUPE / INFOS SEGMENT — bande de cellules (durée, ratio, résolution…) */
.cut-info-row {
...
@media (max-width: 900px) {
  .block-container { padding: 0 1.5rem 1.5rem !important; }
  .specs-row { flex-wrap: wrap !important; }
  .spec-cell { flex: 1 1 45% !important; border-bottom: 1px solid var(--border); }
}
...
@media (max-width: 640px) {
  ...
  [data-testid="stHorizontalBlock"] { row-gap: 1.25rem !important; }
  ...
  .cut-info-cell { flex: 1 1 100% !important; }

  div.stButton > button,
  div.stDownloadButton > button,
  div[data-testid="stDownloadButton"] > button {
    height: 42px !important; font-size: 0.88rem !important;
  }

  .preview-placeholder { min-height: 180px; margin-top: 1rem; }
}
...
def render_video(
    ...
    q = QUALITY_PRESETS.get(
        quality_key, QUALITY_PRESETS["Standard (CRF 18 — recommandé)"]
    )
    cmd = [
        "ffmpeg",
        "-y",
...
def replace_audio(
    video_path: str, audio_path: str, output_path: str, loop_audio: bool = True
):
...
with tab_v:
    ...
    if video_file:
        ...
        with col_ctrl:
            ...
            st.markdown(
                '<p class="section-label-mt">Qualité d\'export</p>',
                unsafe_allow_html=True,
            )
            ...
            if not st.session_state.rendered_bytes:
                if st.button("Générer le rendu", key="vbtn"):
                    ...
                    try:
                        render_video(
                            vp, lp, out, nfo, quality_key=quality_key, **wm_opts
                        )
                        ph.empty()
...
with tab_p:
    ...
    if photo_files:
        ...
        with col_ctrl_p:
            ...
            st.markdown(
                '<p class="section-label-mt">Fichiers importés</p>',
                unsafe_allow_html=True,
            )
        ...
        with col_ctrl_p:
            if len(photo_files) == 1:
                ...
                st.markdown(
                    '<p class="section-label-mt">Téléchargement</p>',
                    unsafe_allow_html=True,
                )
...
with tab_s:
    ...
    if scr_file:
        ...
        with col_ctrl_s:
            ...
            st.markdown(
                '<p class="section-label">Timecode (secondes)</p>',
                unsafe_allow_html=True,
            )
...
with tab_cut:
    ...
    if cut_file:
        ...
        with col_ctrl_c:
            ...
            st.markdown(
                '<p class="section-label-mt">Début du segment</p>',
                unsafe_allow_html=True,
            )
            t_start = st.slider(
                "Début",
            ...
            st.markdown(
                '<p class="section-label-mt">Fin du segment</p>', unsafe_allow_html=True
            )
            t_end = st.slider(
                "Fin",
                min_value=0.0,
            ...
            if not st.session_state.cut_bytes:
                ...
                st.markdown(
                    '<div class="status status-ok">Découpage terminé.</div>',
                    unsafe_allow_html=True,
                )

        with col_prev_c:
            st.markdown(
                '<p class="section-label">Aperçu du segment sélectionné</p>',
                unsafe_allow_html=True,
            )
...
with tab_merge:
    ...
    with col_ctrl_m:
        st.markdown(
            '<p class="section-label">Sources (dans l\'ordre de fusion)</p>',
            unsafe_allow_html=True,
        )
    ...
    if merge_files and len(merge_files) >= 2:
        ...
        with col_ctrl_m:
            st.markdown(
                '<p class="section-label-mt">Fichiers à fusionner</p>',
                unsafe_allow_html=True,
            )
            ...
            if not st.session_state.merge_bytes:
                ...
                st.markdown(
                    '<div class="status status-ok">Fusion terminée.</div>',
                    unsafe_allow_html=True,
                )
...
with tab_audio:
    ...
    if audio_vid_file:
        ...
        with col_ctrl_a:
            ...
            st.markdown(
                '<p class="section-label-mt">Action</p>', unsafe_allow_html=True
            )
            ...
            if "Remplacer" in audio_action:
                ...
                if audio_replace_file:
                    st.markdown(
                        '<p class="section-label-mt">Options</p>',
                        unsafe_allow_html=True,
                    )
                    loop_audio = st.checkbox(
                        "Boucler l'audio si plus court que la vidéo",
                        value=True,
                        key="audio_loop",
                    )
                        ...
                        try:
                            if "Supprimer" in audio_action:
                                ...
                                arp = os.path.join(
                                    tmp_a,
                                    "audio_replace"
                                    + os.path.splitext(audio_replace_file.name)[1],
                                )
...
<p style="font-family:sans-serif;font-size:0.72rem;color:#999;text-align:center;margin:6px 0 0;">
  Mode muet — le son sera supprimé
                ...
                _aud_ext = (
                    os.path.splitext(audio_replace_file.name)[1].lower().lstrip(".")
                )
                _aud_mime_map = {
                    "mp3": "audio/mpeg",
                    "wav": "audio/wav",
...
with tab_crop:
    ...
    if crop_file:
        ...
        with col_ctrl_r:
            ...
            st.markdown(
                '<p class="section-label-mt">Ratio cible</p>', unsafe_allow_html=True
            )
            ...
            st.markdown(
                '<p class="section-label-mt">Position du cadre</p>',
                unsafe_allow_html=True,
            )
            ...
            if not st.session_state.crop_bytes:
                ...
                st.markdown(
                    '<div class="status status-ok">Recadrage terminé.</div>',
                    unsafe_allow_html=True,
                )
        ...
        with col_prev_r:
            ...
            draw2.rectangle(
                [cx, cy, cx + cw - 1, cy + ch - 1], outline=(0, 104, 177), width=3
            )
            st.image(
...
with tab_canva:
    ...
    with col_ctrl_cv:
        ...
        with _cr1:
            st.markdown(
                '<p class="section-label">Arrière-plan</p>', unsafe_allow_html=True
            )
        ...
        with _cr3:
            st.markdown(
                '<p class="section-label">Watermark</p>', unsafe_allow_html=True
            )
            wm_opts_cv = {
                "position": st.selectbox(
                    "Position watermark",
        ...
        _canva_offset_sig = (
            canva_bg_file.name if canva_bg_file else None,
            canva_format,
        )
        ...
        canva_sur = st.text_input(
            "Surtitre",
            value="Modifier le surtitre",
            key="canva_sur",
            label_visibility="collapsed",
        )

        st.markdown(
            '<p class="section-label">Titre principal</p>', unsafe_allow_html=True
        )
        ...
        def generate_canva_image():
            ...
            if canva_bg_file:
                ...
                if dx >= 0 and dy >= 0:
                    ...
                    if crop_w > 0 and crop_h > 0:
                        cropped = bg.crop(
                            (src_x, src_y, src_x + crop_w, src_y + crop_h)
                        )
            ...
            draw.text(
                (sur_x + pad, sur_y + sur_h // 2 - fs_sur // 2),
                canva_sur,
                font=font_sur,
                fill=sc,
            )
            ...
            for i, line in enumerate(lines):
                ...
                ly = block_top + i * lh - (overlap if i > 0 else 0)
                draw.text(
                    (lx + pad, ly + lh // 2 - fs // 2), line, font=font_title, fill=tc
                )
    ...
    with col_prev_cv:
        ...
        if not canva_bg_file:
            ...
            _canva_bg_mime = (
                "image/png"
                if _ext == "png"
                else ("image/webp" if _ext == "webp" else "image/jpeg")
            )
