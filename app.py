...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&family=Roboto+Condensed:wght@400;500;700&display=swap');
...
.site-header-right { font-size: 0.65rem; color: var(--muted); }
...
/* Colonnes */
[data-testid="stHorizontalBlock"] { align-items: flex-start !important; gap: 0.5rem !important; }
[data-testid="stColumn"] { min-height: 0 !important; }
.col-controls {
...
/* Labels de section plus compacts */
.section-label {
  font-size: 0.6rem;
  font-weight: 500;
  color: var(--muted);
...
[data-testid="stFileUploaderDropzoneInstructions"] span { color: var(--sub) !important; font-weight: 500; }
...
[data-testid="stFileUploaderFileName"] { font-size: 0.7rem !important; }
...
div.stButton > button {
  ...
  box-shadow: 0 1px 2px rgba(0,104,177,0.15) !important;
}
...
div.stDownloadButton > button {
  ...
  color: #fff !important;
  box-shadow: 0 1px 2px rgba(22,163,74,0.18) !important;
}
div.stDownloadButton > button:hover { background: #15803d !important; transform: translateY(-1px) !important; }

/* Sliders */
[data-testid="stSlider"] { margin-bottom: 0.2rem !important; padding-bottom: 0 !important; }
...
.photo-batch-dim { font-size: 0.6rem; color: var(--muted); flex-shrink: 0; margin-left: 0.3rem; }
...
.merge-item-dur { font-size: 0.6rem; color: var(--muted); flex-shrink: 0; }
...
.cut-info-cell span { display: block; font-size: 0.5rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.06em; }

/* Status */
.status { font-size: 0.7rem; padding: 0.15rem 0; margin: 0.15rem 0; }
.status-ok   { color: var(--green); }
.status-err  { color: var(--red); }
.status-idle { color: var(--muted); }

/* Encodage spinner */
.encoding-wrap { display: flex; align-items: center; gap: 0.5rem; padding: 0.3rem 0; margin: 0.2rem 0; }
.encoding-ring {
...
.encoding-text { font-size: 0.7rem; color: var(--sub); }
.fake-progress-wrap { margin: 0.2rem 0; }
.fake-progress-track { height: 2px; background: var(--border); border-radius: 99px; overflow: hidden; }
.fake-progress-bar {
  height: 100%;
  border-radius: 99px;
...
def render_video(video_path: str, logo_path: str, output_path: str, info: dict,
                 ...
                 quality_key: str = "Standard (CRF 18)", progress_cb=None):
...
def watermark_options_ui(key_prefix: str) -> dict:
    ...
    if position == "Coordonnées personnalisées":
        ...
        with col_x:
            custom_x = st.number_input("X (px)", min_value=0, value=0, step=1, key=f"{key_prefix}_cx")
        with col_y:
            custom_y = st.number_input("Y (px)", min_value=0, value=0, step=1, key=f"{key_prefix}_cy")
...
tab_v, tab_p, tab_s, tab_cut, tab_merge, tab_audio, tab_crop, tab_canva = st.tabs([
    "🎬 Watermark vidéo", "🖼️ Watermark photo", "📸 Capture",
    "✂️ Couper", "🔗 Fusionner", "🔊 Son", "📐 Recadrer", "🎨 Template RS"
])

# -------------------- WATERMARK VIDÉO --------------------
with tab_v:
    col_ctrl, col_prev = st.columns([4, 6], gap="small")
    ...
    if video_file:
        ...
        with col_ctrl:
            st.markdown(f"""
            <div class="specs-row">
              <div class="spec-cell"><span class="spec-k">Largeur</span><span class="spec-v">{nfo['width']}</span></div>
              <div class="spec-cell"><span class="spec-k">Hauteur</span><span class="spec-v">{nfo['height']}</span></div>
            ...
            with st.expander("⚙️ Paramètres du watermark", expanded=True):
                wm_opts = watermark_options_ui("v")
            with st.expander("📤 Qualité d'export"):
            ...
            if not st.session_state.rendered_bytes:
                if st.button("Générer le rendu", key="vbtn"):
                    ...
                    ph = st.empty()
                    ph.markdown(
                        '<div class="encoding-wrap"><div class="encoding-ring"></div><span class="encoding-text">Encodage…</span></div>'
                        '<div class="fake-progress-wrap"><div class="fake-progress-track"><div class="fake-progress-bar"></div></div></div>',
                        unsafe_allow_html=True
                    )
                    try:
                ...
                st.download_button(
                    "↓ Télécharger", data=st.session_state.rendered_bytes,
        ...
        with col_ctrl:
            st.markdown('<div class="status status-idle">Déposez une vidéo.</div>', unsafe_allow_html=True)
        with col_prev:
            st.markdown("""
            <div class="preview-placeholder">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#0068B1" stroke-width="1.2">
...
# -------------------- WATERMARK PHOTO --------------------
with tab_p:
    col_ctrl_p, col_prev_p = st.columns([4, 6], gap="small")
    ...
    if photo_files:
        ...
        with col_ctrl_p:
            ...
            for pf in photo_files:
                ...
                st.markdown(
                    ...
                    f'<span class="photo-batch-dim">{W_tmp} × {H_tmp}</span>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

            with st.expander("⚙️ Position du watermark", expanded=True):
        ...
        with col_ctrl_p:
            if len(photo_files) == 1:
                ...
                st.download_button("↓ Télécharger", data=data,
        ...
        with col_ctrl_p:
            st.markdown('<div class="status status-idle">Déposez une ou plusieurs images.</div>', unsafe_allow_html=True)
        with col_prev_p:
            st.markdown("""
            <div class="preview-placeholder">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#0068B1" stroke-width="1.2">
...
# -------------------- CAPTURE --------------------
with tab_s:
    col_ctrl_s, col_prev_s = st.columns([4, 6], gap="small")

    with col_ctrl_s:
        st.markdown('<p class="section-label">Source</p>', unsafe_allow_html=True)
        scr_file = st.file_uploader("Déposez votre vidéo", type=["mp4", "mov", "avi", "mkv", "webm"],
    ...
    if scr_file:
        ...
        with col_ctrl_s:
            st.markdown(f"""
            <div class="specs-row">
              <div class="spec-cell"><span class="spec-k">Largeur</span><span class="spec-v">{nfo_s['width']}</span></div>
              <div class="spec-cell"><span class="spec-k">Hauteur</span><span class="spec-v">{nfo_s['height']}</span></div>
            ...
            st.markdown('<p class="section-label">Timecode (s)</p>', unsafe_allow_html=True)
...
# -------------------- COUPER --------------------
with tab_cut:
    col_ctrl_c, col_prev_c = st.columns([4, 6], gap="small")
    ...
    if cut_file:
        ...
        with col_ctrl_c:
            st.markdown(f"""
            <div class="specs-row">
              <div class="spec-cell"><span class="spec-k">Largeur</span><span class="spec-v">{nfo_c['width']}</span></div>
              <div class="spec-cell"><span class="spec-k">Hauteur</span><span class="spec-v">{nfo_c['height']}</span></div>
            ...
            if not st.session_state.cut_bytes:
                if st.button("Générer le découpage", key="cut_btn"):
                    ...
                    ph_c = st.empty()
                    ph_c.markdown(
                        '<div class="encoding-wrap"><div class="encoding-ring"></div><span class="encoding-text">Découpage…</span></div>'
                        '<div class="fake-progress-wrap"><div class="fake-progress-track"><div class="fake-progress-bar"></div></div></div>',
                        unsafe_allow_html=True
                    )
                    try:
        ...
        with col_prev_c:
            ...
            components.html(f"""
            <div style="border:1px solid #e4e4e4;border-radius:8px;overflow:hidden;background:#0a0a0a;">
              <video id="cutplayer" controls style="width:100%;display:block;max-height:360px;object-fit:contain;"
...
# -------------------- FUSIONNER --------------------
with tab_merge:
    col_ctrl_m, col_prev_m = st.columns([4, 6], gap="small")

    with col_ctrl_m:
        st.markdown('<p class="section-label">Sources (ordre de fusion)</p>', unsafe_allow_html=True)
    ...
    if merge_files and len(merge_files) >= 2:
        ...
        with col_ctrl_m:
            ...
            st.markdown(f"""
            <div class="specs-row" style="margin-top:0.4rem;">
            ...
            if not st.session_state.merge_bytes:
                if st.button("Fusionner les vidéos", key="merge_btn"):
                    ...
                    ph_m = st.empty()
                    ph_m.markdown(
                        '<div class="encoding-wrap"><div class="encoding-ring"></div><span class="encoding-text">Fusion…</span></div>'
                        '<div class="fake-progress-wrap"><div class="fake-progress-track"><div class="fake-progress-bar"></div></div></div>',
                        unsafe_allow_html=True
                    )
                    try:
        ...
        with col_ctrl_m:
            st.markdown('<div class="status status-idle">Ajoutez au moins une deuxième vidéo.</div>', unsafe_allow_html=True)
    else:
        with col_ctrl_m:
            st.markdown('<div class="status status-idle">Déposez au moins <b>deux</b> vidéos.</div>', unsafe_allow_html=True)
        with col_prev_m:
            st.markdown("""
            <div class="preview-placeholder">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#0068B1" stroke-width="1.2">
...
# -------------------- SON --------------------
with tab_audio:
    col_ctrl_a, col_prev_a = st.columns([4, 6], gap="small")
    ...
    if audio_vid_file:
        ...
        with open(avp, "wb") as f:
            f.write(audio_vid_file.read())
        nfo_a = get_video_info(avp)

        with col_ctrl_a:
            st.markdown(f"""
            <div class="specs-row">
              <div class="spec-cell"><span class="spec-k">Largeur</span><span class="spec-v">{nfo_a['width']}</span></div>
              <div class="spec-cell"><span class="spec-k">Hauteur</span><span class="spec-v">{nfo_a['height']}</span></div>
            ...
            if "Remplacer" in audio_action:
                ...
                if audio_replace_file:
                    ...
                    loop_audio = st.checkbox("Boucler l'audio si plus court", value=True, key="audio_loop")
            ...
            if not st.session_state.audio_bytes:
                ...
                if can_go:
                    if st.button(btn_lbl, key="audio_btn"):
                        ...
                        ph_a = st.empty()
                        ph_a.markdown(
                            '<div class="encoding-wrap"><div class="encoding-ring"></div><span class="encoding-text">Traitement audio…</span></div>'
                            '<div class="fake-progress-wrap"><div class="fake-progress-track"><div class="fake-progress-bar"></div></div></div>',
                            unsafe_allow_html=True
                        )
                        try:
        ...
        with col_prev_a:
            ...
            if "Supprimer" in audio_action:
                components.html(f"""
                <div style="border:1px solid #e4e4e4;border-radius:8px;overflow:hidden;background:#0a0a0a;">
                  <video controls muted style="width:100%;display:block;max-height:340px;object-fit:contain;"
                         src="data:{_amime};base64,{_ab64}"></video>
                </div>
                <p style="font-size:0.65rem;color:#999;text-align:center;margin:4px 0 0;">Mode muet — le son sera supprimé</p>
                """, height=400)
                ...
                components.html(f"""
                <div style="border:1px solid #e4e4e4;border-radius:8px;overflow:hidden;background:#0a0a0a;">
                  <video id="prev_vid" controls muted style="width:100%;display:block;max-height:340px;object-fit:contain;"
                         src="data:{_amime};base64,{_ab64}"></video>
                </div>
                <audio id="prev_aud" src="data:{_aud_mime};base64,{_aud_b64}" {"loop" if loop_audio else ""}></audio>
                <p style="font-size:0.65rem;color:#999;text-align:center;margin:4px 0 0;">
                ...
                components.html(f"""
                <div style="border:1px solid #e4e4e4;border-radius:8px;overflow:hidden;background:#0a0a0a;">
                  <video controls style="width:100%;display:block;max-height:360px;object-fit:contain;"
                         src="data:{_amime};base64,{_ab64}"></video>
                </div>""", height=400)
...
# -------------------- RECADRER --------------------
with tab_crop:
    col_ctrl_r, col_prev_r = st.columns([4, 6], gap="small")
    ...
    if crop_file:
        ...
        with col_ctrl_r:
            st.markdown(f"""
            <div class="specs-row">
              <div class="spec-cell"><span class="spec-k">Largeur</span><span class="spec-v">{W_r}</span></div>
              <div class="spec-cell"><span class="spec-k">Hauteur</span><span class="spec-v">{H_r}</span></div>
              <div class="spec-cell"><span class="spec-k">Durée</span><span class="spec-v">{fmt_time(nfo_r['duration'])}</span></div>
              <div class="spec-cell"><span class="spec-k">Ratio</span><span class="spec-v">{W_r}:{H_r}</span></div>
            </div>""", unsafe_allow_html=True)
            ...
            src_ratio = W_r / H_r
            if src_ratio > target_ratio:
                out_w = int(H_r * target_ratio)
                out_h = H_r
            else:
                out_w = W_r
                out_h = int(W_r / target_ratio)
            out_w -= out_w % 2
            out_h -= out_h % 2
            ...
            <div class="cut-info-row">
              ...
              <div class="cut-info-cell"><span>Final</span>{out_w}×{out_h}</div>
            ...
            if not st.session_state.crop_bytes:
                if st.button("Recadrer", key="crop_btn"):
                    out_r = os.path.join(tmp_r, "crop_output.mp4")
                    ph_r = st.empty()
                    ph_r.markdown(
                        '<div class="encoding-wrap"><div class="encoding-ring"></div><span class="encoding-text">Recadrage…</span></div>'
                        '<div class="fake-progress-wrap"><div class="fake-progress-track"><div class="fake-progress-bar"></div></div></div>',
                        unsafe_allow_html=True
                    )
                    try:
        ...
        with col_prev_r:
            ...
            if crop_pos == "Haut":
                ...
                cx, cy = (pw - cw) // 2, ph_img - ch
            elif crop_pos == "Gauche":
                cx, cy = 0, (ph_img - ch) // 2
            elif crop_pos == "Droite":
                cx, cy = pw - cw, (ph_img - ch) // 2
            else:
                cx, cy = (pw - cw) // 2, (ph_img - ch) // 2
            ...
            st.image(cap_image_for_preview(prev_r),
                     caption=f"{chosen[0]} — {out_w}×{out_h} — {crop_pos}")
...
with tab_canva:
    ...
    col_ctrl_cv, col_prev_cv = st.columns([4, 6], gap="small")

    # CSS pour réduire l'espace des sliders dans cet onglet
    st.markdown("""
    <style>
    ...
    [data-testid="stColorPicker"] label { font-size: 0.65rem !important; margin-bottom: 2px !important; }
    .cv-reset > button {
      height: 24px !important; min-height: 24px !important;
      font-size: 0.65rem !important; padding: 0 0.5rem !important;
      background: var(--bg) !important;
      border: 1px solid var(--border) !important;
      color: var(--muted) !important;
      border-radius: 999px !important;
    }
    .cv-reset > button:hover { border-color: var(--blue) !important; color: var(--blue) !important; background: var(--blue-dim) !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    with col_ctrl_cv:
        ...
        canva_title = st.text_area(
            "Titre", value="Modifier le titre (↵ pour retour à la ligne)",
            key="canva_title", label_visibility="collapsed", height=70
        )
        ...
        st.markdown('<p class="section-label" style="margin-top:6px;">Position du texte</p>', unsafe_allow_html=True)
        canva_y = st.slider("Position Y", min_value=5, max_value=95, key="canva_y", label_visibility="collapsed")

        if canva_bg_file:
            st.markdown('<p class="section-label" style="margin-top:4px;">Zoom photo</p>', unsafe_allow_html=True)
            canva_img_zoom = st.slider("Zoom", min_value=100, max_value=300, key="canva_imgzoom", label_visibility="collapsed")

            st.markdown('<p class="section-label" style="margin-top:4px;">Décalage horizontal</p>', unsafe_allow_html=True)
            canva_offx = st.slider("Décalage X", min_value=-540, max_value=540, key="canva_offx", label_visibility="collapsed")

            st.markdown('<p class="section-label" style="margin-top:4px;">Décalage vertical</p>', unsafe_allow_html=True)
            canva_offy = st.slider("Décalage Y", min_value=-960, max_value=960, key="canva_offy", label_visibility="collapsed")
        else:
            canva_img_zoom = 100
            canva_offx = 0
            canva_offy = 0
        ...
        def generate_canva_image():
            ...
            def _wrap_segment(text, max_chars=34):
                words = text.split(' ')
                segs, cur = [], ''
                for w in words:
        ...
        st.markdown("""
        <div style="margin-top:0.6rem; padding:0.4rem 0.8rem; background:#e8f2fb; border:1px solid #b3d4f0; border-radius:6px; font-size:0.7rem; color:#0068B1; line-height:1.5;">
          <b>💡</b> Clic droit sur l’aperçu → <code>Enregistrer l’image sous…</code>
        </div>""", unsafe_allow_html=True)
    ...
    with col_prev_cv:
        ...
        if not canva_bg_file:
            ...
            <div class="preview-placeholder">
              ...
              <span>Glissez une image pour l'aperçu</span>
            </div>""", unsafe_allow_html=True)
        else:
            canva_bg_file.seek(0)
            ...
            _preview_h = min(600, int(460 * canva_h / canva_w))
            components.html(f"""<!DOCTYPE html>
<html>
<head><meta charset="UTF-8">
<style>
  html,body{{margin:0;padding:0;background:#eef1f5;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding-top:6px;box-sizing:border-box;height:100%;overflow:hidden;}}
  #canvasWrap{{box-shadow:0 4px 16px rgba(0,0,0,0.15);display:inline-block;}}
...
const AVAIL_H   = {_preview_h} - 44;
const PREVIEW_W = Math.min(Math.floor(AVAIL_H * CANVAS_W / CANVAS_H), window.innerWidth - 16);
...
</html>""", height=_preview_h + 44, scrolling=False)
...
<span>Envoyez-moi <a href="mailto:lucas.bessonnat@leprogres.fr">les messages d'erreur par mail</a>.<br>
  Après plusieurs utilisations, appuyez sur <code>F5</code> pour rafraîchir le cache.</span>
</div>
""", unsafe_allow_html=True)
