...
import subprocess, tempfile, os, math, io, zipfile, shutil, json
...
PREVIEW_MAX_W, PREVIEW_MAX_H = 680, 500
POSITIONS = [
...
@media (max-width: 768px) {
  ...
  .col-preview { padding-left: 0 !important; }
  ...
  div.stButton > button, div.stDownloadButton > button { height: 44px !important; }
}
@media (max-width: 480px) {
  .spec-cell { flex: 1 1 100%; }
}
...
/* LABELS */
.section-label, .section-label-mt {
  font-size: 0.68rem;
  font-weight: 500;
  color: var(--muted);
...
.section-label-mt { margin-top: 1.2rem; }
...
.preview-wrap img { width: 100% !important; height: auto !important; display: block !important; object-fit: contain !important; }
...
/* ENCODING SPINNER */
.encoding-wrap { display: flex; align-items: center; gap: 0.7rem; padding: 0.5rem 0; }
.encoding-ring {
...
.video-preview-wrap video { width: 100%; display: block; max-height: 420px; object-fit: contain; }

/* AUDIO CHIP */
.audio-option-row { display: flex; gap: 0.5rem; margin-bottom: 0.8rem; }
.audio-chip {
...
.audio-chip.active { border-color: var(--blue); background: var(--blue-dim); color: var(--blue); font-weight: 500; }
...
def extract_frame(video_path, timecode):
...
def make_thumbnail(video_path, logo_path, info, position=DEFAULT_POSITION, custom_x=0, custom_y=0):
    result = subprocess.run([
        "ffmpeg", "-y", "-i", video_path,
        "-vframes", "1", "-f", "image2pipe", "-vcodec", "png", "pipe:1"
    ], capture_output=True)
    frame = Image.open(io.BytesIO(result.stdout)).convert("RGBA")
    return composite_logo(frame, logo_path, position=position, custom_x=custom_x, custom_y=custom_y,
                          force_w=info["width"], force_h=info["height"]).convert("RGB")

def render_video(video_path, logo_path, output_path, info, position=DEFAULT_POSITION,
                 custom_x=0, custom_y=0, quality_key="Standard (CRF 18 — recommandé)", progress_cb=None):
...
def trim_video(video_path, output_path, t_start, t_end):
    cmd = [
        "ffmpeg", "-y",
...
def remove_audio(video_path, output_path):
    cmd = ["ffmpeg", "-y", "-i", video_path, "-c:v", "copy", "-an", "-movflags", "+faststart", output_path]
    result = subprocess.run(cmd, capture_output=True)
    if result.returncode != 0:
        raise RuntimeError(result.stderr.decode())

def replace_audio(video_path, audio_path, output_path, loop_audio=True):
...
def crop_video(video_path, output_path, ratio_w, ratio_h, position="Centre"):
...
def watermark_options_ui(key_prefix):
    st.markdown('<p class="section-label-mt">Watermark</p>', unsafe_allow_html=True)
    position = st.selectbox("Position", POSITIONS, index=POSITIONS.index(DEFAULT_POSITION), key=f"{key_prefix}_pos")
    custom_x = custom_y = 0
...
def build_photo_output(pf, logo_path, opts):
    pf.seek(0)
    base = Image.open(pf)
    result = composite_logo(base, logo_path, **opts)
    buf = io.BytesIO()
...
with tab_v:
    ...
    with col_ctrl:
        ...
        video_file = st.file_uploader("Déposez votre vidéo ici", type=["mp4", "mov", "avi", "mkv", "webm"], key="vu", label_visibility="collapsed")
    if video_file:
        if st.session_state.get("_last_video_name") != video_file.name:
            ...
            with col_ctrl:
                ...
                if st.session_state.thumbnail is None:
                    with st.spinner("Génération de l'aperçu…"):
                        st.session_state.thumbnail = make_thumbnail(vp, DEFAULT_WM_FILE, nfo, **wm_opts)
                ...
                if not st.session_state.rendered_bytes:
                    if st.button("Générer le rendu", key="vbtn"):
                        ...
                        try:
                            render_video(vp, DEFAULT_WM_FILE, out, nfo, quality_key=quality_key, **wm_opts)
                            ph.empty()
...
with tab_p:
    ...
    with col_ctrl_p:
        st.markdown('<p class="section-label">Source</p>', unsafe_allow_html=True)
        photo_files = st.file_uploader("Déposez vos images ici", type=["png", "jpg", "jpeg"], key="pu", label_visibility="collapsed", accept_multiple_files=True)
    if photo_files:
        with col_ctrl_p:
            ...
            for pf in photo_files:
                ...
                pf.seek(0)
                st.markdown(f'<div class="photo-batch-item"><span class="photo-batch-name">📷 {pf.name}</span><span class="photo-batch-dim">{W_tmp} × {H_tmp} px</span></div>', unsafe_allow_html=True)
        ...
        with col_prev_p:
            ...
            for idx, pf in enumerate(photo_files):
                ...
                result_prev = composite_logo(base_prev, DEFAULT_WM_FILE, **wm_opts_p)
                with grid_cols[idx % 2]:
                    st.image(cap_image_for_preview(result_prev.convert("RGB")), caption=pf.name, use_container_width=True)
        with col_ctrl_p:
            if len(photo_files) == 1:
                data, fname, mime = build_photo_output(photo_files[0], DEFAULT_WM_FILE, wm_opts_p)
                ...
                for i in range(0, len(photo_files), 2):
                    ...
                    for j, pf in enumerate(row_files):
                        data, fname, mime = build_photo_output(pf, DEFAULT_WM_FILE, wm_opts_p)
                        with btn_cols[j]:
                            st.download_button(f"↓  {pf.name}", data=data, file_name=fname, mime=mime, key=f"pdl_{i+j}")
                zip_buf = io.BytesIO()
                with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_STORED) as zf:
                    for pf in photo_files:
                        data, fname, _ = build_photo_output(pf, DEFAULT_WM_FILE, wm_opts_p)
                        zf.writestr(fname, data)
                st.download_button("↓  Tout télécharger (.zip)", data=zip_buf.getvalue(), file_name="photos_watermark.zip", mime="application/zip", key="pdl_zip")
    else:
        with col_ctrl_p:
...
with tab_s:
    ...
    with col_ctrl_s:
        st.markdown('<p class="section-label">Source</p>', unsafe_allow_html=True)
        scr_file = st.file_uploader("Déposez votre vidéo ici", type=["mp4", "mov", "avi", "mkv", "webm"], key="su", label_visibility="collapsed")
    if scr_file:
        with tempfile.TemporaryDirectory() as tmp:
            sp = os.path.join(tmp, "src" + os.path.splitext(scr_file.name)[1])
            ...
            with col_ctrl_s:
                ...
                st.markdown("<div style='margin-top:1.2rem;'></div>", unsafe_allow_html=True)
                st.download_button("↓  Télécharger la capture", data=buf_s.getvalue(), file_name=f"capture_{fmt_time(timecode).replace(':','-')}.png", mime="image/png", key="sdl")
            with col_prev_s:
                st.markdown('<p class="section-label">Aperçu</p>', unsafe_allow_html=True)
...
with tab_cut:
    ...
    with col_ctrl_c:
        st.markdown('<p class="section-label">Source</p>', unsafe_allow_html=True)
        cut_file = st.file_uploader("Déposez votre vidéo ici", type=["mp4", "mov", "avi", "mkv", "webm"], key="cut_u", label_visibility="collapsed")
    if cut_file:
        if st.session_state.get("_last_cut_name") != cut_file.name:
            st.session_state.cut_bytes = None
            st.session_state._last_cut_name = cut_file.name
        with tempfile.TemporaryDirectory() as tmp:
            cp = os.path.join(tmp, "src" + os.path.splitext(cut_file.name)[1])
            ...
            with col_ctrl_c:
                ...
                t_start = st.slider("Début", min_value=0.0, max_value=float(dur_c), value=0.0, step=0.1, format="%.1f s", key="cut_start", label_visibility="collapsed")
                st.markdown('<p class="section-label-mt">Fin du segment</p>', unsafe_allow_html=True)
                t_end = st.slider("Fin", min_value=0.0, max_value=float(dur_c), value=float(dur_c), step=0.1, format="%.1f s", key="cut_end", label_visibility="collapsed")
                if t_end <= t_start:
                ...
                if not st.session_state.cut_bytes:
                    if st.button("Générer le découpage", key="cut_btn"):
                        out_c = os.path.join(tmp, "cut_output.mp4")
                        ph_c = st.empty()
            ...
            with col_prev_c:
                ...
                with open(cp, "rb") as _vf:
                    _vb64 = _b64h.b64encode(_vf.read()).decode()
...
with tab_merge:
    ...
    with col_ctrl_m:
        ...
        merge_files = st.file_uploader("Déposez vos vidéos ici", type=["mp4", "mov", "avi", "mkv", "webm"], key="merge_u", label_visibility="collapsed", accept_multiple_files=True)
    if merge_files and len(merge_files) >= 2:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for i, mf in enumerate(merge_files):
                mp = os.path.join(tmp, f"src_{i}" + os.path.splitext(mf.name)[1])
            ...
            with col_ctrl_m:
                ...
                for i, (mf, nfo_m) in enumerate(zip(merge_files, nfo_list)):
                    st.markdown(f'<div class="merge-item"><div class="merge-item-idx">{i+1}</div><span class="merge-item-name">🎬 {mf.name}</span><span class="merge-item-dur">{fmt_time(nfo_m["duration"])} — {nfo_m["width"]}×{nfo_m["height"]}</span></div>', unsafe_allow_html=True)
                st.markdown(f"""
                ...
                if not st.session_state.merge_bytes:
                    if st.button("Fusionner les vidéos", key="merge_btn"):
                        out_m = os.path.join(tmp, "fusion_output.mp4")
                        ph_m = st.empty()
                    ...
                    st.download_button("↓  Télécharger la vidéo fusionnée", data=st.session_state.merge_bytes, file_name="fusion.mp4", mime="video/mp4", key="merge_dl")
            ...
            with col_prev_m:
                ...
                for i, (mp, mf) in enumerate(zip(merge_paths, merge_files)):
                    ...
                    st.image(cap_image_for_preview(frame_m), caption=f"{i+1}. {mf.name}", use_container_width=True)
...
with tab_audio:
    ...
    with col_ctrl_a:
        st.markdown('<p class="section-label">Source</p>', unsafe_allow_html=True)
        audio_vid_file = st.file_uploader("Déposez votre vidéo ici", type=["mp4", "mov", "avi", "mkv", "webm"], key="audio_vid_u", label_visibility="collapsed")
    if audio_vid_file:
        if st.session_state.get("_last_audio_name") != audio_vid_file.name:
            st.session_state.audio_bytes = None
            st.session_state._last_audio_name = audio_vid_file.name
        with tempfile.TemporaryDirectory() as tmp:
            avp = os.path.join(tmp, "src" + os.path.splitext(audio_vid_file.name)[1])
            ...
            with col_ctrl_a:
                ...
                audio_action = st.radio("Action audio", ["Supprimer le son", "Remplacer par..."], key="audio_action", label_visibility="collapsed")
                ...
                if "Remplacer" in audio_action:
                    ...
                    audio_replace_file = st.file_uploader("Déposez votre fichier audio", type=["mp3", "wav", "aac", "m4a", "ogg"], key="audio_replace_u", label_visibility="collapsed")
                ...
                if not st.session_state.audio_bytes:
                    ...
                    if can_go:
                        if st.button(btn_lbl, key="audio_btn"):
                            out_a = os.path.join(tmp, "audio_output.mp4")
                            ph_a = st.empty()
                            ...
                            try:
                                if "Supprimer" in audio_action:
                                    ...
                                    arp = os.path.join(tmp, "audio_replace" + os.path.splitext(audio_replace_file.name)[1])
                    ...
                    st.download_button("↓  Télécharger la vidéo", data=st.session_state.audio_bytes, file_name="video_audio_modifie.mp4", mime="video/mp4", key="audio_dl")
                    st.markdown('<div class="status status-ok">Audio traité avec succès.</div>', unsafe_allow_html=True)
            with col_prev_a:
                st.markdown('<p class="section-label">Aperçu</p>', unsafe_allow_html=True)
                with open(avp, "rb") as _vf:
                    _ab64 = _b64h.b64encode(_vf.read()).decode()
...
<p style="font-family:sans-serif;font-size:0.72rem;color:#999;text-align:center;margin:6px 0 0;">
  Mode muet — le son sera supprimé
                    ...
                    _aud_b64 = _b64h.b64encode(_aud_bytes).decode()
                    _aud_ext = os.path.splitext(audio_replace_file.name)[1].lower().lstrip(".")
                    _aud_mime_map = {"mp3": "audio/mpeg", "wav": "audio/wav", "aac": "audio/aac", "m4a": "audio/mp4", "ogg": "audio/ogg"}
...
with tab_crop:
    ...
    with col_ctrl_r:
        st.markdown('<p class="section-label">Source</p>', unsafe_allow_html=True)
        crop_file = st.file_uploader("Déposez votre vidéo ici", type=["mp4", "mov", "avi", "mkv", "webm"], key="crop_u", label_visibility="collapsed")
    if crop_file:
        if st.session_state.get("_last_crop_name") != crop_file.name:
            st.session_state.crop_bytes = None
            st.session_state._last_crop_name = crop_file.name
        with tempfile.TemporaryDirectory() as tmp:
            crp = os.path.join(tmp, "src" + os.path.splitext(crop_file.name)[1])
            ...
            with col_ctrl_r:
                ...
                preset_choice_idx = st.radio("Ratio", options=list(range(len(CROP_PRESETS))),
                ...
                crop_pos = st.selectbox("Position", ["Centre", "Haut", "Bas", "Gauche", "Droite"], key="crop_pos", label_visibility="collapsed")
                st.markdown(f"""
                <div class="cut-info-row">
                ...
                if not st.session_state.crop_bytes:
                    if st.button("Recadrer la vidéo", key="crop_btn"):
                        out_r = os.path.join(tmp, "crop_output.mp4")
                        ph_r = st.empty()
                    ...
                    st.download_button("↓  Télécharger la vidéo recadrée", data=st.session_state.crop_bytes,
                                       file_name=f"recadre_{chosen[0].replace(':','x')}.mp4", mime="video/mp4", key="crop_dl")
            ...
            with col_prev_r:
                ...
                st.image(cap_image_for_preview(prev_r), caption=f"{chosen[0]} — {out_w}×{out_h} px — {crop_pos}")
    else:
        with col_ctrl_r:
            st.markdown('<div class="status status-idle">Déposez une vidéo via <i>Upload</i>.</div>', unsafe_allow_html=True)
        with col_prev_r:
...
with tab_canva:
    ...
    with col_ctrl_cv:
        ...
        with _cr1:
            ...
            canva_bg_file = st.file_uploader("Déposez votre image ici", type=["png", "jpg", "jpeg", "webp"], key="canva_bg", label_visibility="collapsed")
        with _cr2:
            st.markdown('<p class="section-label">Format</p>', unsafe_allow_html=True)
            canva_format = st.selectbox("Format", ["1080×1350 — Portrait", "1080×1080 — Carré", "1080×1920 — Stories"], key="canva_format", label_visibility="collapsed")
        with _cr3:
            st.markdown('<p class="section-label">Watermark</p>', unsafe_allow_html=True)
            wm_opts_cv = {
                "position": st.selectbox("Position watermark", POSITIONS, index=POSITIONS.index(DEFAULT_POSITION), key="cv_pos", label_visibility="collapsed"),
                "custom_x": 0,
                "custom_y": 0,
            }
        ...
        _fmt_map = {"1080×1350 — Portrait": (1080, 1350), "1080×1080 — Carré": (1080, 1080), "1080×1920 — Stories": (1080, 1920)}
        ...
        if st.session_state.get("_canva_offset_sig") != _canva_offset_sig:
            ...
            st.session_state["_canva_offset_sig"] = _canva_offset_sig
        ...
        """, unsafe_allow_html=True)
        canva_offset_raw = st.text_input("canva_offset_bridge", key="canva_offset", label_visibility="collapsed")
        try:
            _cox_s, _coy_s = canva_offset_raw.split(",")
            canva_off_x = float(_cox_s)
            canva_off_y = float(_coy_s)
        except Exception:
        ...
        canva_title = st.text_area("Titre", value="Modifier le titre (➡️ il maintenant possible de faire des retours à la ligne grâce à la touche Entrée)", key="canva_title", label_visibility="collapsed", height=80)
...
let bgOffX = INIT_OFF_X, bgOffY = INIT_OFF_Y;
...
window.addEventListener('mouseup',()=>{{
  ...
  syncOffsetToStreamlit();
}});
if(bgImg) canvas.style.cursor='grab';

function syncOffsetToStreamlit() {{
  try {{
    const bridge = window.parent.document.querySelector('input[aria-label="canva_offset_bridge"]');
    if (!bridge) return;
    const newVal = Math.round(bgOffX) + ',' + Math.round(bgOffY);
    if (bridge.value === newVal) return;
    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.parent.HTMLInputElement.prototype, 'value').set;
    bridge.focus();
    nativeInputValueSetter.call(bridge, newVal);
