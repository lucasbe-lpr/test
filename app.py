...
from utils import (
    get_video_info, fmt_time, extract_frame, make_thumbnail,
    composite_logo, render_video, trim_video, merge_videos,
    remove_audio, replace_audio, crop_video,
    cap_image_for_preview, get_default_logo,
    POSITIONS, DEFAULT_POSITION, QUALITY_PRESETS, CROP_PRESETS,
    cleanup_temp_dir
)
...
# ---- CSS personnalisé (on garde le même, avec suppression des émojis dans le texte) ----
st.markdown("""
<style>
...
# ---- Initialisation session_state pour les chemins de fichiers générés ----
for k in ["_tmp_dirs", "rendered_path", "_last_video_name",
          "cut_path", "_last_cut_name", "merge_path",
          "audio_path", "_last_audio_name", "crop_path", "_last_crop_name"]:
...
# ---- Fonction utilitaire pour nettoyer les anciens temporaires ----
def clean_old_temp_dirs():
    for d in st.session_state._tmp_dirs:
        cleanup_temp_dir(d)
    st.session_state._tmp_dirs = []

def register_temp_dir(path):
    st.session_state._tmp_dirs.append(path)

# ---- Fonction watermark_options_ui (déplacée ici car UI) ----
...
with tab_v:
    ...
    if video_file:
        # Nettoyer les anciens temporaires si nouveau fichier
        ...
        vp = os.path.join(tmp_dir, "src" + os.path.splitext(video_file.name)[1])
        with open(vp, "wb") as f:
            f.write(video_file.read())

        # Mise en cache de get_video_info
        ...
        with col_ctrl:
            ...
            # Signature pour régénérer l'aperçu si les options changent
            ...
            if not st.session_state.rendered_path:
                if st.button("Générer le rendu", key="vbtn"):
                    ...
                    try:
                        ...
                        ph.empty()
                        st.session_state.rendered_path = out
                        st.rerun()
                    except Exception as e:
                        ph.markdown(f'<div class="status status-err">Erreur : {e}</div>', unsafe_allow_html=True)
            else:
                with open(st.session_state.rendered_path, "rb") as f:
                    video_bytes = f.read()
                st.download_button(
        ...
        with col_ctrl_p:
            ...
            for pf in photo_files:
                ...
                st.markdown(
                    ...
                    f'<span class="photo-batch-name">Image : {pf.name}</span>'
...
with tab_cut:
    ...
    if cut_file:
        ...
        with col_ctrl_c:
            ...
            if t_end <= t_start:
                st.markdown('<div class="status status-err">Attention : la fin doit être après le début.</div>', unsafe_allow_html=True)
            ...
            if not st.session_state.cut_path:
        ...
        with col_ctrl_m:
            ...
            for i, (mf, nfo_m) in enumerate(zip(merge_files, nfo_list)):
                st.markdown(
                    ...
                    f'<span class="merge-item-name">Vidéo : {mf.name}</span>'
            ...
            if st.session_state.get("_merge_sig") != merge_sig:
                st.session_state.merge_path = None
                st.session_state["_merge_sig"] = merge_sig

            if not st.session_state.merge_path:
                ...
                if can_go:
                    if st.button(btn_lbl, key="audio_btn"):
                        ...
                        try:
                            ...
                            ph_a.empty()
                            st.session_state.audio_path = out_a
                            st.rerun()
                        except Exception as e:
...
with tab_crop:
    ...
    if crop_file:
        ...
        with col_ctrl_r:
            ...
            if not st.session_state.crop_path:
...
# ============================================================
# ONGLET TEMPLATE RS (Canva) – version simplifiée (on garde l'existant)
# ============================================================
with tab_canva:
    try:
    ...
    # Petit CSS pour réduire les marges des sliders
    st.markdown("""
    <style>
    ...
    # Initialisation des états pour les sliders
    ...
    with col_ctrl_cv:
        ...
        canva_title = st.text_area(
            "Titre", value="Modifier le titre (il est maintenant possible de faire des retours à la ligne grâce à la touche Entrée)",
