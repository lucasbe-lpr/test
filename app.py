...
# Ajout des offsets pour l'onglet Canva
_CANVA_OFFSET_DEFAULTS = {"canva_offx": 0, "canva_offy": 0}
for k, v in _CANVA_OFFSET_DEFAULTS.items():
    if k not in st.session_state:
...
with tab_canva:
    ...
    # Styles supplémentaires pour les widgets et champs cachés
    st.markdown("""
    <style>
    ...
    # Initialisation des offsets (déjà faite en début de script, mais on s'assure qu'ils existent)
    _CANVA_OFFSET_DEFAULTS = {"canva_offx": 0, "canva_offy": 0}
    for _k, _v in _CANVA_OFFSET_DEFAULTS.items():
        if _k not in st.session_state:
            st.session_state[_k] = _v
  ...
  Pour télécharger le visuel, <b>faites un clic droit sur l'aperçu</b> puis sélectionnez <code>Enregistrer l'image sous…</code>
    ...
    with col_prev_cv:
        ...
        if not canva_bg_file:
            ...
            # Récupération des offsets actuels depuis session_state
            _offx = st.session_state.canva_offx
            _offy = st.session_state.canva_offy
...
// Fonction pour mettre à jour les champs cachés de Streamlit
function updateOffsets() {{
  // Champ offx
  const inpX = window.parent.document.querySelector('[data-testid="stTextInput"] input[aria-label="offx"]');
  if (inpX) {{
    const nativeInputSetter = Object.getOwnPropertyDescriptor(window.parent.HTMLInputElement.prototype, 'value').set;
    nativeInputSetter.call(inpX, String(bgOffX));
    inpX.dispatchEvent(new Event('input', {{ bubbles: true }}));
  }}
  // Champ offy
  const inpY = window.parent.document.querySelector('[data-testid="stTextInput"] input[aria-label="offy"]');
  if (inpY) {{
    const nativeInputSetter = Object.getOwnPropertyDescriptor(window.parent.HTMLInputElement.prototype, 'value').set;
    nativeInputSetter.call(inpY, String(bgOffY));
    inpY.dispatchEvent(new Event('input', {{ bubbles: true }}));
  }}
}}
...
window.addEventListener('mouseup',()=>{{
  if(isDragging) {{
    ...
    clampOffset();
    render();
    updateOffsets();  // <-- Sauvegarde des offsets dans Streamlit
    exportCanvas();
  }}
}});
